...
//                 continue;
//             }
//             assert!(!(p_start < pp_end && pp_start < p_end), "Buddy allocator returned
...
//         let ptr = buddy_allocator_alloc(BUDDY_ALLOCATOR_SIZE / 8).map_err(|_| "Allocation failed
...
//     let ptr = buddy_allocator_alloc(BUDDY_ALLOCATOR_SIZE).map_err(|_| "Could not allocate full
...
//     let p1 = buddy_allocator_alloc(PAGE_SIZE).map_err(|_| "Could not allocate full size of Buddy
// Allocator buffer")?;     let p2 = buddy_allocator_alloc(PAGE_SIZE).map_err(|_| "Could not
